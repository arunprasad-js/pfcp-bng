...
*/
#ifndef FILE_BNGC_PFCP_ASSOCIATION_HPP_SEEN
#define FILE_BNGC_PFCP_ASSOCIATION_HPP_SEEN
...
// Adaptation of pgw_pfcp_association.hpp for pfcp bng
...
public:
    ...
    // TODO: Add BBF UP features extension
    ...
    pfcp_association(const pfcp::node_id_t& node_id,
            pfcp::recovery_time_stamp_t& recovery_time_stamp) :
            node_id(node_id), recovery_time_stamp(recovery_time_stamp),
            m_sessions(), sessions() {
        hash_node_id = std::hash<pfcp::node_id_t>{}(node_id);
        timer_heartbeat = ITTI_INVALID_TIMER_ID;
        num_retries_timer_heartbeat = 0;
...
private:
    ...
    pfcp_associations() : associations(PFCP_MAX_ASSOCIATIONS) {};
...
#endif /* FILE_BNGC_PFCP_ASSOCIATION_HPP_SEEN */
