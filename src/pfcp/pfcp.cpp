...
pfcp_l4_stack::pfcp_l4_stack(const string& ip_address, const unsigned short port_num, const util::thread_sched_params& sched_params) :
    ...
    trxn_id2seq_num(MAX_TIMER_N), proc_cleanup_timers(MAX_TIMER_N), msg_out_retry_timers(MAX_TIMER_N), pending_procedures(MAX_TIMER_N) {
...
void pfcp_l4_stack::handle_receive_message_cb(const pfcp_msg& msg, const endpoint& remote_endpoint, const task_id_t& task_id, bool &error, uint64_t& trxn_id)
{
  trxn_id = 0;
  error = true;
  folly::AtomicHashMap<uint32_t , pfcp_procedure>::iterator it;
...
{
  folly::AtomicHashMap<uint64_t , uint32_t>::iterator it;
  ...
  if (it != trxn_id2seq_num.end()) {
    ...
    if (a == DELETE_TX) {
      folly::AtomicHashMap<uint32_t , pfcp_procedure>::iterator it_proc = pending_procedures.find(it->second);
      if (it_proc != pending_procedures.end()) {
        stop_proc_cleanup_timer(it_proc->second);
        pending_procedures.erase(it->second);
      }
...
{
  handled = false;
  folly::AtomicHashMap<timer_id_t, uint32_t>::iterator it = msg_out_retry_timers.find(timer_id);
  if (it != msg_out_retry_timers.end()) {
    folly::AtomicHashMap<uint32_t , pfcp_procedure>::iterator it_proc = pending_procedures.find(it->second);
    msg_out_retry_timers.erase(timer_id);
    handled = true;
    if (it_proc != pending_procedures.end()) {
    ...
    if (it != proc_cleanup_timers.end()) {
      folly::AtomicHashMap<uint32_t , pfcp_procedure>::iterator it_proc = pending_procedures.find(it->second);
      proc_cleanup_timers.erase(timer_id);
      handled = true;
      if (it_proc != pending_procedures.end()) {
