...
typedef enum {
  ...
  TASK_SPGWU_S1U,
  TASK_SGW_UDP,
  TASK_BNGC_APP, // BNG control plane app
  TASK_BNGC_PFCP, // BNG CP PFCP interface
  TASK_BNGU_APP, // BNG user plane app
  TASK_BNGU_PFCP, // BNG UP PFCP interface
...
typedef enum {
  ...
  UDP_DATA_REQ,
  UDP_DATA_IND,
  BNGU_DPDK_SEND_MESSAGE_REQUEST, // Request to send dpdk command in telnet CLI
  NEW_REDIS_MSG, // New messaged received by redis client
  TIME_OUT,
  HEALTH_PING,
  TERMINATE,
