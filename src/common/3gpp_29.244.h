...
public:
  ...
  pfcp_ie_exception(std::string& aphrase) throw() {
    phrase = aphrase;
  }
...
public:
  pfcp_ie_unimplemented_exception(uint16_t ie_type) throw() : pfcp_ie_exception(ie_type) {
...
public:
  pfcp_tlv_bad_length_exception(uint16_t ie_type, uint16_t ie_length, const char* file, const int line) throw() : pfcp_tlv_exception(ie_type){
...
public:
  pfcp_ie_value_exception(uint16_t ie_type, const char* field) throw() : pfcp_ie_exception(ie_type){
...
//  };
  ...
  struct up_function_features_s {
    ...
    //  return *this;
    //}
    up_function_features_s() : bucp(0), ddnd(0), dlbd(0), trst(0),
    ftup(0), pfdm(0), heeu(0), treu(0),
    empu(0), pdiu(0), udbc(0), quoac(0),
    trace(0), frrt(0), spare(0) {}
  ...
  typedef struct metric_s {
    uint8_t  metric; // It indicates a percentage and may take binary coded integer values from
    // and including 0 up to and including 100. Other values shall be considered as 0.
  ...
  typedef struct fseid_s {
    ...
    bool operator<(const struct fseid_s& i) const {
      if (i.seid<this->seid) return true;
      else if (i.seid>this->seid) return false;
      if (i.v4==this->v4) {
        if (i.ipv4_address.s_addr > this->ipv4_address.s_addr) return true;
        else if (i.ipv4_address.s_addr < this->ipv4_address.s_addr) return false;
      } else if (this->v4) return true;
      if (i.v6==this->v6) {
  ...
  struct node_id_s {
    ...
    bool operator==(const struct node_id_s& i) const {
        if (
            (i.node_id_type==this->node_id_type) && (i.u1.ipv4_address.s_addr==this->u1.ipv4_address.s_addr) && (i.fqdn == this->fqdn) &&
  ...
  struct cp_function_features_s {
    ...
    cp_function_features_s(const cp_function_features_s& i) : spare(i.spare), ovrl(i.ovrl), load(i.load) {}
  };
  ...
  typedef struct ue_ip_address_s {
    uint8_t         ipv6d:1; // This bit is only applicable to the UE IP address IE in the PDI IE and whhen V6 bit is set to "1". If this bit is set to "1", then the IPv6 Prefix Delegation Bits field shall be present, otherwise the UP function shall consider IPv6 prefix is default /64.
    uint8_t         sd:1; // This bit is only applicable to the UE IP Address IE in the PDI IE. It shall be set to "0" and ignored by the receiver in IEs other than PDI IE. In the PDI IE, if this bit is set to "0", this indicates a Source IP address; if this bit is set to "1", this indicates a Destination IP address.
    uint8_t         v4:1; // If this bit is set to "1", then the IPv4 address field shall be present in the UE IP Address, otherwise the IPv4 address field shall not be present.
    uint8_t         v6:1; // If this bit is set to "1", then the IPv6 address field shall be present in the UE IP Address, otherwise the IPv6 address field shall not be present.
    struct in_addr  ipv4_address;
    struct in6_addr ipv6_address;
...
//  typedef struct user_plane_path_failure_report_s {
...
//  typedef struct ethernet_packet_filter_s {
...
//  typedef struct create_traffic_endpoint_s {
//    traffic_endpoint_id_t       traffic_endpoint_id;
//    fteid_t                     local_fteid;
//    network_instance_t          network_instance;
//    ue_ip_address_t             ue_ip_address;
...
//  typedef struct overload_control_information_s {
...
//  typedef struct remove_traffic_endpoint_s {
//    traffic_endpoint_id_t       traffic_endpoint_id;
...
//  typedef struct usage_report_within_pfcp_session_modification_response_s {
//    urr_id_t                    urr_id;
//    ur_seqn_t                   ur_seqn;
...
//  typedef struct downlink_data_report_s {
//    pdr_id_t                    pdr_id;
...
//  typedef struct application_detection_information_s {
//    application_id_t                    application_id;
...
//  typedef struct error_indication_report_s {
...
//    dl_buffering_suggested_packet_count_t dl_buffering_buffering_suggested_packet_count;
//    suggested_buffering_packets_count_t   suggested_buffering_packets_count;
//  } update_bar_within_pfcp_session_report_response_t;

// BBF PFCP IE extensions
  typedef struct pppoe_session_id_s {
    uint16_t pppoe_session_id;
  } pppoe_session_id_t;

#define BBF_OUTER_HEADER_REMOVAL_ETHERNET           1
#define BBF_OUTER_HEADER_REMOVAL_PPPOE_ETHERNET     2
#define BBF_OUTER_HEADER_REMOVAL_PPP_PPPOE_ETHERNET 3
#define BBF_OUTER_HEADER_REMOVAL_L2TP               4
#define BBF_OUTER_HEADER_REMOVAL_PPP_L2TP           5

  typedef struct bbf_outer_header_removal_s {
    uint16_t    bbf_outer_header_removal_description;
  } bbf_outer_header_removal_t;

enum bbf_outer_header_creation_description_value_e {
   BBF_OUTER_HEADER_CREATION_CPR_NSH=0x0100,
   BBF_OUTER_HEADER_CREATION_TRAFFIC_ENDPOINT=0x0200,
   BBF_OUTER_HEADER_CREATION_L2TP=0x0400,
   BBF_OUTER_HEADER_CREATION_PPP=0x0800
  };

  typedef struct bbf_outer_header_creation_s {
    uint16_t    bbf_outer_header_creation_description;
    uint16_t    tunnel_id;
    uint16_t    session_id;
  } bbf_outer_header_creation_t;
}
